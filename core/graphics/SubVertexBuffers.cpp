...
namespace Liar
{
	...
	* Int
	*/
	SubIntVertexBuffer::SubIntVertexBuffer() :
		Liar::ISubVertexBuffers(),
		m_buffers(nullptr), m_numberBuffers(0)
	{}
	...
	Liar::Int SubIntVertexBuffer::GetSize() const
	{
		return  m_buffers ? m_buffers[0]->GetSize() : 0;
	}

	Liar::Int SubIntVertexBuffer::GetFormat() const
	{
		return  m_buffers ? m_buffers[0]->GetNumberBuffer() : 0;
	}

	void SubIntVertexBuffer::AddVertexBuffer(void* c)
	{
		void* buffer = GetSubVertexBuffer(Liar::VertexFormatType::VERTEX_FORMAT_TYPE_INT, (void**)m_buffers, m_numberBuffers, c);
		if (!buffer)
		{
			SetVertexBufferLen(m_numberBuffers + 1);
			m_buffers[m_numberBuffers - 1] = static_cast<Liar::IntHeapOperator*>(c);
		}
	}

	void SubIntVertexBuffer::SetVertexBuffer(Liar::Int index, void* color)
	{
		if (m_buffers[index] != color && m_buffers[index]) delete m_buffers[index];
		m_buffers[index] = static_cast<Liar::IntHeapOperator*>(color);
	}

	void SubIntVertexBuffer::SetVertexBufferLen(Liar::Int len)
	{
		if (len == m_numberBuffers) return;
		Liar::Int i = 0;
		if (len < m_numberBuffers)
		...
		{
			size_t blockSize = sizeof(Liar::IntHeapOperator*)*len;
			if (m_buffers) m_buffers = (Liar::IntHeapOperator**)realloc(m_buffers, blockSize);
			else m_buffers = (Liar::IntHeapOperator**)malloc(blockSize);
	...
	void* SubIntVertexBuffer::GetVertexBuffer(Liar::Int index) const
	{
		if (index >= m_numberBuffers) return nullptr;
		return m_buffers[index];
	}

	void SubIntVertexBuffer::ToString()
	{
		std::cout << "intHeapOperator num:" << m_numberBuffers << std::endl;
		for (Liar::Int i = 0; i < m_numberBuffers; ++i)
		{
			std::cout << "index:" << i << " ";
			std::cout << *(m_buffers[i]) << std::endl;
	...
	* float
	*/
	SubFloatVertexBuffer::SubFloatVertexBuffer() :
		Liar::ISubVertexBuffers(),
		m_buffers(nullptr), m_numberBuffers(0)
	{}
	...
	Liar::Int SubFloatVertexBuffer::GetSize() const
	{
		return  m_buffers ? m_buffers[0]->GetSize() : 0;
	}

	Liar::Int SubFloatVertexBuffer::GetFormat() const
	{
		return  m_buffers ? m_buffers[0]->GetNumberBuffer() : 0;
	}

	void SubFloatVertexBuffer::AddVertexBuffer(void* c)
	{
		void* buffer = GetSubVertexBuffer(Liar::VertexFormatType::VERTEX_FORMAT_TYPE_FLOAT, (void**)m_buffers, m_numberBuffers, c);
		if (!buffer)
		{
			SetVertexBufferLen(m_numberBuffers + 1);
			m_buffers[m_numberBuffers - 1] = static_cast<Liar::FloatHeapOperator*>(c);
		}
	}

	void SubFloatVertexBuffer::SetVertexBuffer(Liar::Int index, void* color)
	{
		if (m_buffers[index] != color && m_buffers[index]) delete m_buffers[index];
		m_buffers[index] = static_cast<Liar::FloatHeapOperator*>(color);
	}

	void SubFloatVertexBuffer::SetVertexBufferLen(Liar::Int len)
	{
		if (len == m_numberBuffers) return;
		Liar::Int i = 0;
		if (len < m_numberBuffers)
		...
		{
			size_t blockSize = sizeof(Liar::FloatHeapOperator*)*len;
			if (m_buffers) m_buffers = (Liar::FloatHeapOperator**)realloc(m_buffers, blockSize);
			else m_buffers = (Liar::FloatHeapOperator**)malloc(blockSize);
	...
	void* SubFloatVertexBuffer::GetVertexBuffer(Liar::Int index) const
	{
		if (index >= m_numberBuffers) return nullptr;
		return m_buffers[index];
	}

	void SubFloatVertexBuffer::ToString()
	{
		std::cout << "floatHeapOperator num:" << m_numberBuffers << std::endl;
		for (Liar::Int i = 0; i < m_numberBuffers; ++i)
		{
			std::cout << "index:" << i << " ";
			std::cout << *(m_buffers[i]) << std::endl;
